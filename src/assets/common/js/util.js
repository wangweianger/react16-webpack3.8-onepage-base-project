        ...
        $('#expInputFile').one('change',function(){
            ...
            for(var i=0;i<files.length;i++){
                ...
                for(var k=0;k<checktype.length;k++){
                    var checked=new RegExp('\.'+checktype[k]+'$').test(files[i]['name']) // eslint-disable-line
                    if(!checked){continue}
                    begin=checked
                }
                if(!begin){
	...
	*/
    regCombination(type,numArr){
        ...
        case 'm':    //"m":"写手机号码！"
            ...
            break	
        case 'e':   //"e":"邮箱地址格式
            reg=new RegExp("^\\w+([-+.']\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*$")   // eslint-disable-line
            break
        case 'id':   //'id':验证身份证
    ...
    getNumListDouHao(json) {
        ...
        for (var i = 0; i < json.datas.length; i++) {
            if (i == json.datas.length - 1) {
                for (let k = 0; k < json.keys.length; k++) {
